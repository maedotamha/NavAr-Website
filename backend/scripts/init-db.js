...
const statements = [
  ...
  "INSERT INTO permissions (module_id, action_key, permission_key, name) SELECT m.id, a.action_key, m.module_key || '.' || a.action_key, a.name FROM permission_modules m CROSS JOIN (VALUES ('read','Read'),('create','Create'),('update','Edit'),('delete','Delete')) AS a(action_key, name) WHERE m.module_key IN ('facilities','maps','qr_anchors','routes','access_control') ON CONFLICT (permission_key) DO NOTHING",
