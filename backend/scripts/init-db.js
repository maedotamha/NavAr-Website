...
const statements = [
  ...
  "DO $$ BEGIN IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='navigation_sessions' AND column_name='success') THEN UPDATE navigation_sessions SET session_status = CASE WHEN success = FALSE THEN 'cancelled' WHEN status IN ('cancelled','canceled','not_completed') THEN 'cancelled' WHEN status = 'completed' THEN 'completed' ELSE session_status END WHERE session_status IS NULL; ELSE UPDATE navigation_sessions SET session_status = CASE WHEN status IN ('cancelled','canceled','not_completed') THEN 'cancelled' WHEN status = 'completed' THEN 'completed' ELSE session_status END WHERE session_status IS NULL; END IF; END $$",
